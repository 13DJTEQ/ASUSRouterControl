...
**Operator rule:** primary instructions are always a **complete one-shot** (curl → Desktop → run). Never start from `./install-to-desktop.sh` or `./hermes-desktop-repair.sh` unless a one-shot in the same paste just installed the drop. See [`.cursor/rules/one-shot-operator-scripts.mdc`](../.cursor/rules/one-shot-operator-scripts.mdc).
...
1. Download the full `hermes-desktop-ops` toolset onto `~/Desktop/hermes-desktop-ops` (collector, monitor, repair, libs)
2. Run the collector
...
### Repair (dry-run default) — one-shot primary
...
Paste **as one line**. This refreshes the full Desktop toolset (including repair), then runs dry-run:
...
Apply only after reviewing dry-run output (paste as one line):

```bash
curl -fsSL https://raw.githubusercontent.com/13DJTEQ/ASUSRouterControl/cursor/hermes-desktop-ops-cfe4/hermes-desktop-ops/bootstrap-desktop-collector.sh -o ~/Desktop/bootstrap-desktop-collector.sh && chmod +x ~/Desktop/bootstrap-desktop-collector.sh && /bin/bash ~/Desktop/bootstrap-desktop-collector.sh --skip-collect && /bin/bash ~/Desktop/hermes-desktop-ops/hermes-desktop-repair.sh --apply
```

If the Desktop drop was just installed in this session:
...
Secondary path only — prefer the one-shot above on operator Macs.
