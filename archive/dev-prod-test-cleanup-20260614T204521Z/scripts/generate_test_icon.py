...
def _render_png(path: Path, size: int) -> None:
    ...
    text = NSAttributedString.alloc().initWithString_attributes_("TEST", attrs)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate a red TEST .icns app icon.")
    parser.add_argument(
        "--output",
        required=True,
    ...
    with tempfile.TemporaryDirectory(prefix="test-iconset-") as tmp:
        iconset_dir = Path(tmp) / "test.iconset"
