...
async def probe_wired_clients(
    ssh: RouterSSH,
    *,
    wifi_macs: set[str] | None = None,
) -> list[dict]:
    """Discover likely wired clients on br0 using bridge + neighbor tables."""
    wifi_set = {
        _normalize_mac(mac)
        for mac in (wifi_macs or set())
        if isinstance(mac, str) and mac.strip()
    }

    bridge_ports: dict[str, int] = {}
    wifi_ports: set[int] = set()
    try:
        bridge_rows = await ssh.run("brctl showmacs br0 2>/dev/null")
        if bridge_rows.ok and bridge_rows.stdout:
            for line in bridge_rows.stdout.splitlines():
                match = _BRIDGE_MAC_ROW_RE.search(line)
                if not match:
                    continue
                if match.group("local").lower() == "yes":
                    continue
    ...
    try:
        neigh_rows = await ssh.run(
            "ip neigh show dev br0 2>/dev/null || ip neigh show 2>/dev/null"
        )
        if neigh_rows.ok and neigh_rows.stdout:
            for line in neigh_rows.stdout.splitlines():
                match = _NEIGH_ROW_RE.search(line)
                if not match:
                    continue
                dev = (match.group("dev") or "br0").strip()
                if dev != "br0":
                    continue
                state_raw = (match.group("state") or "").strip()
    ...
    for mac in sorted(candidates):
        ...
        if wifi_ports and port is not None and port in wifi_ports:
            continue
        ip, state = neigh_by_mac.get(mac, (None, None))
        wired.append({
            "mac": mac.upper(),
            "ip": ip,
            "bridge_port": port,
