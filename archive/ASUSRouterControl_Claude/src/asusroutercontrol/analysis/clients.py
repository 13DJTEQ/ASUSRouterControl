...
async def get_client_load_summary(store: DataStore) -> list[dict]:
    ...
    Wired devices are always included in full — they are few and bounded by
    physical LAN ports.  Wireless clients are capped at 20 (sorted by signal
    quality then load) so the menu stays usable on home networks.
    """
    ...
    # Always include all wired devices; cap only wireless at 15.
    wired = sorted(
        [row for row in combined if _is_wired_band(row.get("band"))],
        key=_row_rank, reverse=True,
    )
    wireless = sorted(
        [row for row in combined if not _is_wired_band(row.get("band"))],
        key=_row_rank, reverse=True,
    )
    return wireless[:20] + wired
