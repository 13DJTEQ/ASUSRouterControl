...
def _profile_field(profile_key: str, field: str) -> str:
    """Fetch a field from a named DHCP profile (loaded from TOML)."""
    ...
    if not profile:
        raise click.ClickException(f"Unknown DHCP reservation profile: {profile_key!r}. "
                                   f"Run 'asusrouter dhcp profile list' to see available profiles.")
    if isinstance(profile, DhcpProfile):
        # Map field names to DhcpProfile attributes / properties
        attr_map = {
            "mac": "mac",
            "hostname": "hostname",
    ...
    # Legacy dict fallback (should not normally occur)
...
def dhcp_profile_list():
    ...
    if not profiles:
        ...
        return
    from rich.table import Table
    table = Table(title="DHCP Reservation Profiles", show_header=True, header_style="bold cyan")
...
def dhcp_profile_show(key: str):
    ...
    profile = _get_dhcp_profiles().get(key)
    if not profile:
...
def dhcp_profile_install(overwrite: bool):
    """Install the example profiles file to ~/.asusroutercontrol/dhcp_profiles.toml."""
    ...
    console.print(
        "Edit this file to customise your device shortcuts, then run 'dhcp profile list'."
    )
...
@click.option(
    ...
    default=None,
    show_default=True,
    help="Reserved IP for MacPro12Core (default from dhcp_profiles.toml).",
)
@click.option(
    "--mac",
    default=None,
    show_default=True,
    help="MAC override for Mac Pro profile (default from dhcp_profiles.toml).",
)
@click.option(
    "--hostname",
    default=None,
    show_default=True,
    help="Hostname override for Mac Pro profile (default from dhcp_profiles.toml).",
)
@click.option(
    "--dry-run/--apply",
    default=True,
...
@click.option(
    ...
    default=None,
    show_default=True,
    help="Reserved IP for Denon second ethernet endpoint (default from dhcp_profiles.toml).",
)
@click.option(
    "--mac",
    default=None,
    show_default=True,
    help="MAC override for Denon second-port profile (default from dhcp_profiles.toml).",
)
@click.option(
    "--hostname",
    default=None,
    show_default=True,
    help="Hostname override for Denon second-port profile (default from dhcp_profiles.toml).",
)
@click.option(
    "--dry-run/--apply",
    default=True,
