...
def _linreg(xs: list[float], ys: list[float]) -> tuple[float, float, float]:
    ...
    ss_xy = sum((x - x_bar) * (y - y_bar) for x, y in zip(xs, ys, strict=False))
    if ss_xx == 0:
...
def _iqr_filter(xs: list[float], ys: list[float]) -> tuple[list[float], list[float]]:
    ...
    lo = q1 - 1.5 * iqr
    hi = q3 + 1.5 * iqr
    filtered = [(x, y) for x, y in zip(xs, ys, strict=False) if lo <= y <= hi]
    if len(filtered) < max(3, int(len(ys) * 0.6)):
        return xs, ys
    fx, fy = zip(*filtered, strict=False)
...
async def analyze_trends(store: DataStore, *, days: int = 30) -> dict:
    ...
    if len(dl_pts) >= 3:
        raw_xs, raw_ys = zip(*dl_pts, strict=False)
    ...
    if len(ul_pts) >= 3:
        raw_xs, raw_ys = zip(*ul_pts, strict=False)
        xs, ys = _iqr_filter(list(raw_xs), list(raw_ys))
        slope, _, r2 = _linreg(xs, ys)
        mbps_per_week = slope * 604800 / 1_000_000
    ...
    if len(lat_pts) >= 3:
        raw_xs, raw_ys = zip(*lat_pts, strict=False)
        xs, ys = _iqr_filter(list(raw_xs), list(raw_ys))
        slope, _, r2 = _linreg(xs, ys)
    ...
    for band in ("2.4", "5"):
        ...
        if len(pts) >= 3:
            raw_xs, raw_ys = zip(*pts, strict=False)
            xs, ys = _iqr_filter(list(raw_xs), list(raw_ys))
            slope, _, r2 = _linreg(xs, ys)
    ...
    if len(ram_pts) >= 3:
        raw_xs, raw_ys = zip(*ram_pts, strict=False)
        xs, ys = _iqr_filter(list(raw_xs), list(raw_ys))
        slope, _, r2 = _linreg(xs, ys)
    ...
    if len(jitter_pts) >= 3:
        raw_xs, raw_ys = zip(*jitter_pts, strict=False)
        xs, ys = _iqr_filter(list(raw_xs), list(raw_ys))
        slope, _, r2 = _linreg(xs, ys)
        ms_per_week = slope * 604800
    ...
    if len(temp_pts) >= 3:
        raw_xs, raw_ys = zip(*temp_pts, strict=False)
        xs, ys = _iqr_filter(list(raw_xs), list(raw_ys))
        slope, _, r2 = _linreg(xs, ys)
    ...
    if len(util_pts) >= 3:
        raw_xs, raw_ys = zip(*util_pts, strict=False)
        xs, ys = _iqr_filter(list(raw_xs), list(raw_ys))
        slope, _, r2 = _linreg(xs, ys)
        pct_per_week = slope * 604800
