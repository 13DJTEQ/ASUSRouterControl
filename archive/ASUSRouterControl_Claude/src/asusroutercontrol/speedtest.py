...
# Providers whose results reflect dedicated throughput test infrastructure.
# CDN providers (cachefly, cloudfront, fastly, http_download) measure CDN
# bandwidth, not ISP capacity — they consistently underreport on high-speed
# connections due to small transfer sizes and single-stream limitations.
# Only PRIMARY providers are used to compute the composite download/upload.
_PRIMARY_PROVIDERS = {"ookla", "cloudflare"}
...
class MultiSourceSpeedTest:
    ...
    def _compute_composite(
        ...
        *,
        ...
        # Primary providers (Ookla, Cloudflare) measure actual ISP throughput.
        # CDN providers underreport on high-speed links — use as supplemental only.
        primary_ok = [r for r in ok if r.provider in _PRIMARY_PROVIDERS]
        composite_pool = primary_ok if primary_ok else ok  # fall back if no primary succeeded

        # Median download (primary providers only)
        dl_vals = [r.download_bps for r in composite_pool if r.download_bps]
        dl = median(dl_vals) if dl_vals else None

        # Median upload (primary providers only)
        ul_vals = [r.upload_bps for r in composite_pool if r.upload_bps]
        ...
        # Confidence + outliers (based on primary providers only)
        outliers = self._detect_outliers(dl_vals, composite_pool)
        confidence = self._compute_confidence(dl_vals)

        # Build provider details JSON
        composite_method = "median(primary)" if primary_ok else "median(all-fallback)"
        provider_details = {"providers": {}, "composite_method": composite_method}
        for r in results:
            entry: dict = {}
