...
class MonitorScheduler:
    ...
    async def _collect_client_traffic(self, ssh: RouterSSH) -> None:
        ...
        for snap in snapshots:
            mac_key = str(snap["mac"]).lower()
            mac = mac_key.upper()
            rx_bytes = snap["rx_bytes"]
            tx_bytes = snap["tx_bytes"]
            prev = self._client_prev.get(mac_key)

            self._client_prev[mac_key] = (rx_bytes, tx_bytes, now)
            ...
            if now.tzinfo is not None:
                if prev_ts.tzinfo is None or prev_ts.tzinfo.utcoffset(prev_ts) is None:
                    ...
                    prev_ts = prev_ts.astimezone(now.tzinfo)
            elif prev_ts.tzinfo is not None and prev_ts.tzinfo.utcoffset(prev_ts) is not None:
            ...
            cl = ClientLoad(
                timestamp=now,
                mac=mac,
                hostname=row.get("hostname") if row else None,
                band=band,
                rssi=rssi,
            ...
            inserted_valid += 1
        wifi_macs = {
            str(snap.get("mac") or "").lower()
            for snap in snapshots
            if snap.get("mac")
        }
        for wired in await probe_wired_clients(ssh, wifi_macs=wifi_macs):
            mac = str(wired.get("mac") or "").upper()
            if not mac:
                continue
            row = device_rows.get(mac.lower(), {})
            hostname = row.get("hostname")
            ip_addr = wired.get("ip") or row.get("ip")
            await self._store.upsert_device(
                Device(
                    mac=mac,
                    ip=ip_addr,
                    hostname=hostname,
            ...
            await self._store.insert_device_perf(
                ClientLoad(
                    timestamp=now,
                    mac=mac,
                    hostname=hostname,
