"""ASUSRouterControl CLI entrypoint.

This module re-exports the cli Click group and all commands.
Shared utilities live in cli.core.
"""
...
def dhcp_profile_list():
    ...
    for key, profile in sorted(profiles.items()):
        table.add_row(key, profile.label, profile.mac, profile.default_ip, profile.hostname)
    console.print(table)
...
def dhcp_profile_show(key: str):
    ...
    profile = _get_profiles_for_display().get(key)
    if not profile:
...
@click.option("--overwrite", is_flag=True, help="Overwrite existing user profiles file.")
def dhcp_profile_install(overwrite: bool):
    """Install example profiles to ~/.asusroutercontrol/dhcp_profiles.toml."""
    from asusroutercontrol.dhcp_profiles import install_user_profiles

    cfg = load_config()
    path = install_user_profiles(cfg.data_dir, overwrite=overwrite)
    console.print(f"[green]Profiles file installed:[/green] {path}")
    console.print("Edit this file, then run 'asusrouter dhcp profile list'.")
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Non-interactive: unlock using a shared/Grok Keychain MP candidate, "
        "then copy the proven-good secret into the canonical asusroutercontrol path."
    ),
)
@click.option(
    "--force",
...
def credentials_bw_master(
    ...
    flags = sum(bool(x) for x in (do_set, do_delete, do_status, do_import))
    if flags != 1:
        raise click.UsageError(
            "Specify exactly one of --set, --delete, --status, or --import-from-keychain"
        )
    ...
    if do_status:
        ...
        if err and (
            ...
            console.print(
                "[yellow]Note:[/yellow] Decryption failed means the "
                "asusroutercontrol Keychain value is not the real Bitwarden "
                "master password — Grok/shared services were also searched. "
                "Not a BW outage."
            )
            if not stored or (isinstance(candidates, int) and candidates == 0):
                console.print(
                    ...
                    "(copies a working Grok/shared Keychain MP after unlock test)"
                )
                ...
                console.print(
                    "[dim]Keychain Access: look for universal-keychain-grok-*-"
                    "bw_master_password or universal-keychain-shared-*-"
                    "bw_master_password (items Grok Bots already use). "
                    "Rare typing repair: asusrouter credentials bw-master "
                    "--force --set[/dim]"
                )
            ...
            console.print(
                "[dim]If Keychain Access shows a working Grok/shared item, look for "
                "universal-keychain-grok-*-bw_master_password, "
                "universal-keychain-shared-*-bw_master_password, or "
                "universal-keychain-asusroutercontrol-*-bw_master_password "
                "and re-run --import-from-keychain / --status after pulling this fix "
                "— do not type the master password unless zero candidates exist.[/dim]"
            )
        return
    if do_import:
        console.print(
            "[cyan]Importing Bitwarden master password from shared/Grok Keychain "
            "candidates (non-interactive)…[/cyan]"
        )
        ...
        if info.get("ok"):
            ...
            console.print(
                "[dim]Shared/Grok source item was left intact. "
                "Later unlocks use Keychain only (no typing).[/dim]"
            )
            return
    ...
    console.print(
        "[dim]Prefer --import-from-keychain when Grok Bots already have a working "
        "Keychain MP. Confirm once below only if no shared candidate exists. "
        "After success, status / bw_sync / probe / DEV.app will not ask for the "
        "master password.[/dim]"
    )
...
def _normalize_service_environment(environment: str) -> str:
    ...
    if not env:
        env = "prod"
    if not re.fullmatch(r"[a-z0-9][a-z0-9._-]*", env):
        raise click.BadParameter("Environment must match [a-z0-9][a-z0-9._-]*")
    return env


def _scoped_launchd_label(base_label: str, environment: str) -> str:
    env = _normalize_service_environment(environment)
    return base_label if env == "prod" else f"{base_label}.{env}"


def _scoped_launchd_plist_path(base_label: str, environment: str) -> Path:
    env = _normalize_service_environment(environment)
    filename = f"{base_label}.plist" if env == "prod" else f"{base_label}.{env}.plist"
    return Path.home() / "Library" / "LaunchAgents" / filename


def _validate_env_file(env_file: Path | None) -> Path | None:
    if env_file is None:
        return None
    if not env_file.exists():
        raise click.BadParameter(f"Env file not found: {env_file}")
    return env_file
...
@click.option(
    ...
    default="prod",
    show_default=True,
    help="Launchd environment scope (prod keeps legacy label/path).",
)
@click.option(
    "--env-file",
    type=click.Path(exists=True, dir_okay=False, path_type=Path, resolve_path=True),
    default=None,
    help="Explicit .env file for this launchd service.",
)
def menubar_install(deploy_environment: str, env_file: Path | None):
    ...
    plist_path = _scoped_launchd_plist_path(base_label, deploy_environment)
...
"""
    ...
    if boot.returncode != 0:
        raise click.ClickException(boot.stderr.strip() or f"Failed to bootstrap {label}")
...
@click.option(
    ...
    default="prod",
    show_default=True,
    help="Launchd environment scope to uninstall.",
)
def menubar_uninstall(deploy_environment: str):
    ...
    subprocess.run(["launchctl", "bootout", f"gui/{uid}", str(plist_path)], check=False)
    plist_path.unlink()
    console.print(f"[green]Uninstalled:[/green] {plist_path} ({label})")
...
@click.option(
    ...
    default="prod",
    show_default=True,
    help="Launchd environment scope to restart after rebuild.",
)
def menubar_build(deploy_environment: str):
    ...
    if was_loaded:
        ...
        console.print(f"[green]Menubar app restarted:[/green] {label}")
...
@click.option(
    ...
    default="prod",
    show_default=True,
    help="Launchd environment scope to check.",
)
def menubar_status(deploy_environment: str):
    ...
    if not plist_path.exists():
        console.print(
            "[dim]Not installed. Run: "
            f"asusrouter menubar install --environment {deploy_environment}[/dim]"
        )
        return
    ...
    if result.returncode == 0:
        ...
        console.print(f"Start with: launchctl bootstrap gui/$(id -u) {plist_path}")
...
@click.option(
    ...
    default="prod",
    show_default=True,
    help="Launchd environment scope to diagnose.",
)
@click.option(
    "--env-file",
    type=click.Path(exists=True, dir_okay=False, path_type=Path, resolve_path=True),
    default=None,
    help="Explicit .env file used to resolve runtime data/log paths.",
)
@click.option("--fix", is_flag=True, help="Attempt to re-bootstrap the service if it's unloaded.")
def menubar_doctor(deploy_environment: str, env_file: Path | None, fix: bool):
    ...
    if not plist_path.exists():
        ...
        console.print(
            "  Fix: [bold]"
            f"asusrouter menubar install --environment {deploy_environment}[/bold]"
        )
    ...
    # 3. Python runtime exists?
    python_bin = Path(sys.executable).resolve()
    if python_bin.exists():
        console.print(f"[green]\u2713 Python runtime exists:[/green] {python_bin}")
    else:
        console.print(f"[red]\u2717 Python runtime missing:[/red] {python_bin}")
        issues += 1
    ...
    if python_bin.exists():
        ...
        import_check = subprocess.run(
            [str(python_bin), "-c", "import objc; import AppKit; print('ok')"],
            capture_output=True,
            text=True,
            env=import_env,
        )
...
@click.option(
    ...
    type=click.Path(exists=True, dir_okay=False, path_type=Path, resolve_path=True),
    default=None,
    help="Explicit .env file for scheduler runtime config.",
)
def scheduler_start(env_file: Path | None):
    ...
    env_file = _validate_env_file(env_file)
    if env_file is not None:
        os.environ["ASUSROUTERCONTROL_ENV_FILE"] = str(env_file)
...
def scheduler_install(deploy_environment: str, env_file: Path | None):
...
<dict>
    ...
    <array>
        ...
        <string>-u</string>
        <string>-m</string>
        <string>asusroutercontrol.cli</string>
...
def scheduler_uninstall(deploy_environment: str):
...
def scheduler_status(deploy_environment: str):
    ...
    if not plist_path.exists():
        console.print(
            "[dim]Not installed. Run: "
            f"asusrouter scheduler install --environment {deploy_environment}[/dim]"
        )
        return
...
@click.option("--json", "as_json", is_flag=True, help="Output as JSON")
@click.option("--export", "export_path", type=click.Path(), help="Export dashboard JSON to file")
def dashboard(
    hours: int,
    clients: int,
    timeline_points: int,
    ...
    if as_json:
        ...
        if timeline:
            ...
            console.print("[dim]No speed tests in lookback window for timeline context.[/dim]")

    if export_path:
        export_file = Path(export_path)
        export_file.parent.mkdir(parents=True, exist_ok=True)
        export_file.write_text(json_mod.dumps(data, indent=2, default=str))
