...
async def get_client_load_summary(store: DataStore) -> list[dict]:
    ...
    Keep a bounded menu size while preserving wired visibility: include wired
    rows first (by rank), then fill the remaining slots with top wireless rows.
    """
    max_rows = 15
    ...
    for mac, row in best_fallback.items():
        if mac not in best_valid:
            combined.append(row)
    # Keep wired entries visible while respecting the global row cap.
    wired = sorted(
    ...
    if len(wired) >= max_rows:
        return wired[:max_rows]
    return wireless[: max_rows - len(wired)] + wired
