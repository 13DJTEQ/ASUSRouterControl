...
class AsusWrtBackend(FirmwareBackend):
    """HTTP backend for stock AsusWRT and AsusWRT-Merlin.

    Uses the `asusrouter` library. Shared endpoints for stock and Merlin;
    Merlin-only features are gated via ``flavor`` capability flags.
    """
    ...
    def __init__(
        ...
        *,
        ...
        flavor_norm = (flavor or "merlin").strip().lower()
        if flavor_norm not in {"stock", "merlin"}:
            raise ValueError(f"Unsupported AsusWrtBackend flavor: {flavor!r}")
        self.flavor = flavor_norm
    ...
    @property
    def supports_jffs(self) -> bool:
        return self.flavor == "merlin"

    @property
    def supports_entware(self) -> bool:
        return self.flavor == "merlin"

    @property
    def supports_ssh_probes(self) -> bool:
        return self.flavor == "merlin"

    def require_capability(self, name: str) -> None:
        """Raise BackendOperationUnsupported when a Merlin-only capability is missing."""
        from asusroutercontrol.backends.base import BackendOperationUnsupported

        mapping = {
            "jffs": self.supports_jffs,
            "entware": self.supports_entware,
            "ssh_probes": self.supports_ssh_probes,
        }
        if name not in mapping:
            raise ValueError(f"Unknown capability: {name!r}")
        if not mapping[name]:
            raise BackendOperationUnsupported(
                f"Capability {name!r} requires Merlin flavor; current flavor={self.flavor!r}"
            )
